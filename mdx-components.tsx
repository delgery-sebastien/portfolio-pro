...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    wrapper: ({ children }) => (
      <article className="prose prose-invert max-w-none container pt-10">{children}</article>
    ),
    ...
    table: (props) => (
      <div className="overflow-x-auto my-4 rounded-xl border border-card">
        <table className="min-w-full text-sm" {...props} />
      </div>
    ),
    thead: (props) => (
      <thead
        className="bg-gradient-to-r from-brand-700 to-brand-800 border-b border-slate-700 text-slate-100"
        {...props}
      />
    ),
    th: (props) => <th className="px-4 py-2 text-left font-semibold" {...props} />,
    td: (props) => <td className="px-4 py-2 text-base-normal" {...props} />,

    // 3) Blocs de code lisibles
    pre: (props) => (
      <pre
        className="overflow-x-auto rounded-xl border border-card bg-card p-4 text-sm"
        {...props}
      />
    ),
    ...
    blockquote: (props) => (
      <blockquote
        className="border-l-4 border-emerald-400/60 pl-4 italic text-base-normal"
        {...props}
      />
    ),

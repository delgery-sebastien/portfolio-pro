...
export default function SearchPage(){
  ...
  const { index, byId } = useMemo(() => {
    ...
    docs.forEach(d => { (idx as any).add(d.id, `${d.title} ${d.description||''}`); map.set(d.id, d) })
      ...
      <div className="mt-6 space-y-3">
        {results.map(r=>(
          <Link key={r.slug} href={r.slug} className="block card p-4 hover:border-slate-600">
            ...
            {r.description && <p className="text-slate-300 mt-1">{r.description}</p>}
          </Link>
        ))}
        {!q && <p className="text-slate-400">Astuce : tape “VBA”, “DLL”, “Power BI”…</p>}
        {q && results.length===0 && <p className="text-slate-400">Aucun résultat.</p>}
      </div>
    </section>
  )
}
